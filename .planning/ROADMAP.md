...
4. **Confirm-before-burn + in-flight tracking + reconcile** — `services.py`: `_melt_pay` background task with tristate settlement (`pay_invoice` raises → `check_payment_status` → `paid=True` finalize, `paid=False` restore, `paid=None` leave pending). `_confirm_payment` retry-with-backoff. `_in_flight_melts` refcount registry (registered before task starts, cleared in `finally`). `reconcile_pending_melts` (skips in-flight, resolves stranded notes). `tasks.py` + `lnurlmint_start`/`lnurlmint_stop` wiring via `create_permanent_unique_task`. No-secret-logging on `/w`/`/w/cb`/`/verify` paths.
5. **Critical PoC tests** — Port and pass against LNbits fixtures: `test_poc_duplicate_melt.py` (TEST-01), `test_poc_a2_settle_race.py` (TEST-02), `test_melt_restore_double_payout_poc.py` (TEST-03 — tristate), `test_poc_reconcile_inflight_race.py` (TEST-04), `test_poc_f2_pending_info_leak.py` (TEST-05). Use fake backend returning `paid=None` to exercise tristate.
