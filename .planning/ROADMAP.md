...
3. **Sunset mode + collision griefing + fee conservation PoCs** ✅ — Sunset: `/p/cb` (Phase 2) and `/w/cb` split branch reject with `{"status":"ERROR","reason":"This mint is sunsetting - ..."}`; rotate/merge/melt unaffected (ECON-05). `fresh_secret()` helper in conftest.py. Ported `test_poc_fee_conservation.py` (9 tests — white-box Ledger proving `paid_in == outstanding + melted_out + fees - refunds`, `attacker_gain <= 0`). Ported `test_poc_fee_loop.py` (6 tests — CreateMint bounds, `_min_sendable_msat` termination, iteration cap RuntimeError). Ported `test_poc_a1_collision_griefing.py` (6 tests — swap collision-checks both `mints_records` AND `notes`, squat rejected atomically, victim mint materializes). All 29 tests pass (8 existing + 21 new). *(Plan 03-03 complete — 2026-08-28)*
